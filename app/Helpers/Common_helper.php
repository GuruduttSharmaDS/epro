<?php
 ...
 class Common_helper{
    ...
    public function getTotalRating($id = 0)
    {
    ...
    public function getTotalJobDoneByGaurd($id = 0)
    {
      
        return DB::table("fp_user_hire")
         ->where('valid_upto', 'like', date('Y-m-d').'%')
         ->where('user_id', '=', $id)
         ->get()->count();
    } 
    public function getTotalProfileView($id = 0)
    {
        return DB::table("fp_user_profile_views")
         ->where('user_id', '=', $id)
