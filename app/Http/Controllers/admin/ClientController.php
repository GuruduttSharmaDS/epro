...
class ClientController extends Controller
{
    ...
    public function edit($id){
        ...
        $client = ($id > 0)?Client::with(['auth_detail'])->find($id):array();
        if(empty($client)){
    ...
    public function save(Request $request){
       ...
       if(!empty($id)){
        ...
        if ($validator->fails()){
            if($id > 0){
                return redirect("/dashboard/edit_client_data/".$id)->withErrors($validator)->withInput();
            }else{
        ...
        if($id >0 ){
             return redirect("/dashboard/edit_client_data/".$id);
        }else{
